...
int main(int argc, char *argv[])
{
	...
	FD_ZERO(&readset);
		...
		number = 0;
		memset(bufferino,0,sizeof(bufferino));
		fd--;
		var = FullRead(socket_c_server,bufferino,sizeof(bufferino));
		puts("sono qui\n\n");
		number = atoi(bufferino);
		printf("numero = %d",number);
		puts("fatto bufferino");
		
		
		FullWrite(socket_c_server," ",strlen(" "));
		if (number){
			puts("prima della recv");
			recv(socket_c_server,&ricezione,number*size_struct,MSG_WAITALL);
			puts("dopo la recv");
